...
-- Function to calculate the distance between the local player and the target
local function getDistanceToTarget(targetPlayer)
    ...
    if targetPosition then
        return (LocalPlayer.Character.HumanoidRootPart.Position - targetPosition).Magnitude
    end
    return math.huge  -- Return a very large number if no target is found
end

-- Function to decide the firing delay based on the distance range
local function getFiringDelay(distance)
    if distance <= getgenv().Ranges.CloseDistance then
        return 0.005  -- Fast firing for close targets
    elseif distance <= getgenv().Ranges.MidDistance then
        return 0.01  -- Moderate delay for mid-range targets
    elseif distance <= getgenv().Ranges.FarDistance then
        return 0.02  -- Longer delay for far targets
    else
        return 0.05  -- Default delay for very far targets or out of range
    end
end
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                local distance = getDistanceToTarget(TargetPlayer)

                -- Get the predicted position based on target's velocity
                ...
                if predictedPosition and isWithinBoxFOV(predictedPosition) then
                    ...
                    -- Get the appropriate firing delay based on distance
                    local firingDelay = getFiringDelay(distance)

                    -- Fire if enough time has passed
                    if os.clock() - lastClickTime >= firingDelay then
